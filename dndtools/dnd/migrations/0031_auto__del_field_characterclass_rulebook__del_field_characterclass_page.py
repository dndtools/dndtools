...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Removing unique constraint on 'CharacterClass', fields ['rulebook', 'name']
        db.delete_unique('dnd_characterclass', ['rulebook_id', 'name'])

        # Deleting field 'CharacterClass.rulebook'
        db.delete_column('dnd_characterclass', 'rulebook_id')

        # Deleting field 'CharacterClass.page'
        db.delete_column('dnd_characterclass', 'page')

        # Adding unique constraint on 'CharacterClass', fields ['name']
        db.create_unique('dnd_characterclass', ['name'])


    def backwards(self, orm):
        
        # Removing unique constraint on 'CharacterClass', fields ['name']
        db.delete_unique('dnd_characterclass', ['name'])

        # User chose to not deal with backwards NULL issues for 'CharacterClass.rulebook'
        raise RuntimeError("Cannot reverse this migration. 'CharacterClass.rulebook' and its values cannot be restored.")

        # Adding field 'CharacterClass.page'
        db.add_column('dnd_characterclass', 'page', self.gf('django.db.models.fields.PositiveSmallIntegerField')(null=True, blank=True), keep_default=False)

        # Adding unique constraint on 'CharacterClass', fields ['rulebook', 'name']
        db.create_unique('dnd_characterclass', ['rulebook_id', 'name'])


    models = {
        'dnd.characterclass': {
            'Meta': {'ordering': "['name']", 'unique_together': "(('name',),)", 'object_name': 'CharacterClass'},
