...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Removing unique constraint on 'Domain', fields ['rulebook', 'name']
        db.delete_unique('dnd_domain', ['rulebook_id', 'name'])

        # Deleting field 'Domain.rulebook'
        db.delete_column('dnd_domain', 'rulebook_id')

        # Deleting field 'Domain.page'
        db.delete_column('dnd_domain', 'page')

        # Removing index on 'Domain', fields ['name']
        db.delete_index('dnd_domain', ['name'])

        # Adding unique constraint on 'Domain', fields ['name']
        db.create_unique('dnd_domain', ['name'])


    def backwards(self, orm):
        
        # Removing unique constraint on 'Domain', fields ['name']
        db.delete_unique('dnd_domain', ['name'])

        # Adding index on 'Domain', fields ['name']
        db.create_index('dnd_domain', ['name'])

        # User chose to not deal with backwards NULL issues for 'Domain.rulebook'
        raise RuntimeError("Cannot reverse this migration. 'Domain.rulebook' and its values cannot be restored.")

        # Adding field 'Domain.page'
        db.add_column('dnd_domain', 'page', self.gf('django.db.models.fields.PositiveSmallIntegerField')(null=True, blank=True), keep_default=False)

        # Adding unique constraint on 'Domain', fields ['rulebook', 'name']
        db.create_unique('dnd_domain', ['rulebook_id', 'name'])
