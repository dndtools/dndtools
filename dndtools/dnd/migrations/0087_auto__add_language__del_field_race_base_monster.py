...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.create_table('dnd_language', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('name', self.gf('django.db.models.fields.CharField')(max_length=64)),
            ('slug', self.gf('django.db.models.fields.SlugField')(unique=True, max_length=32, db_index=True)),
        ))
        db.send_create_signal('dnd', ['Language'])

        # Deleting field 'Race.base_monster'
        db.delete_column('dnd_race', 'base_monster_id')

        # Adding M2M table for field automatic_languages on 'Race'
        db.create_table('dnd_race_automatic_languages', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('race', models.ForeignKey(orm['dnd.race'], null=False)),
            ('language', models.ForeignKey(orm['dnd.language'], null=False))
        ))
        db.create_unique('dnd_race_automatic_languages', ['race_id', 'language_id'])

        # Adding M2M table for field bonus_languages on 'Race'
        db.create_table('dnd_race_bonus_languages', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('race', models.ForeignKey(orm['dnd.race'], null=False)),
            ('language', models.ForeignKey(orm['dnd.language'], null=False))
        ))
        db.create_unique('dnd_race_bonus_languages', ['race_id', 'language_id'])


    def backwards(self, orm):
        
        # Deleting model 'Language'
        db.delete_table('dnd_language')

        # Adding field 'Race.base_monster'
        db.add_column('dnd_race', 'base_monster', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['dnd.Monster'], null=True, blank=True), keep_default=False)

        # Removing M2M table for field automatic_languages on 'Race'
        db.delete_table('dnd_race_automatic_languages')

        # Removing M2M table for field bonus_languages on 'Race'
        db.delete_table('dnd_race_bonus_languages')
